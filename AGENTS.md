...
Antaeus is at the repository-foundation stage. Do not claim that an implementation, release, installation path, compatibility guarantee, or empirical result exists until it is present and verifiable in this repository. Do not add stack-dependent product code until the relevant public language and contract decisions are accepted.
...
- Do not force-push, rewrite shared history, merge a pull request, or deploy unless the human explicitly authorizes that action.
...
Keep changes within the public scope described in `README.md` and repository-visible accepted decisions. Do not copy private roadmaps, unpublished research plans, customer information, internal delivery estimates, or private service architecture into this repository.
